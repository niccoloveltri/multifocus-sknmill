...
module correct.multifocus.Iso1 where
...
++rf++ : ∀ {Δ₀ Δ₁ Γ B₀ B₁ C Ξ Ξ' s}
       (fs : All (λ ΔB → ─ ∣ proj₁ ΔB ⇑ proj₂ ΔB) ((Δ₀ , B₀) ∷ Ξ)) →
       (gs : All (λ ΔB → ─ ∣ proj₁ ΔB ⇑ proj₂ ΔB) ((Δ₁ , B₁) ∷ Ξ')) →
       (h : s ⇛rf Γ ； C) → 
       ++rf Δ₀ (Ξ ++ (Δ₁ , B₁) ∷ Ξ') s h (fs ++All gs)
         ≡  ++rf Δ₁ Ξ' s (++rf Δ₀ Ξ s h fs) gs
++rf++ fs gs Ir = refl
++rf++ {Δ₁} {Δ₂} {Ξ = Ξ₁} {Ξ'} fs gs (⊗r+ Δ₀ Ξ m h gs₁ refl) =
  cong (λ x → ⊗r+ Δ₀ (Ξ ++ (Δ₁ , _) ∷ Ξ₁ ++ (Δ₂ , _) ∷ Ξ') m h x refl) (sym (++Allass gs₁ fs gs))
++rf++ fs gs blurr = refl
...
++⊗r+⇓Q (focr s rf f refl) fs gs =
  cong (λ x → focr s x f refl) (++rf++ fs gs rf)
