...
only-rf⇑N-lf⇑P Γ Γ₀ Γ₁ Γ₂ Δ (⊸r f) ℓ =
  trans (only-rf⇑N-lf⇑P Γ Γ₀ Γ₁ Γ₂ (Δ ∷ʳ _) f ℓ)
        (congfoc (congfocl refl (congfocr refl (cong (unfoc _) {!!}))))
only-rf⇑N-lf⇑P Γ Γ₀ Γ₁ Γ₂ Δ (Il q f) ℓ =
  trans (only-rf⇑N-lf⇑P Γ Γ₀ Γ₁ Γ₂ Δ f (Il-1 ℓ))
        (congfoc (congfocl refl (congfocr refl (cong (unfoc _) (cong (MMF.⊸r⋆⇑ Δ) (cong (MMF.runL ℓ) {!!}))))))
...
only-rf-fN-lf-fP .(Γ₅' ++ A' ∷ Ω') Γ₀ .(Ξ ++ Ξ') Γ₂ Δ (focl {_} {_} {_} {_} {.(map _ Γ₅')} q lf (focr {_} {_} {_} {_} {_} {.((∘ , A') ∷ map _ Ω' ++ map (λ z → ∘ , z) Ξ)} {.(map (λ z → ∘ , z) Ξ' ++ map (λ z → ∘ , z) Δ)} (just (M , m)) rf (unfoc ok f) refl refl ξ₁) refl refl ξ) refl ℓ | inj₁ (.(map (λ z → ∘ , z) Ξ) , refl , refl) | inj₂ (.(∘ , A') , .(map _ Ω') , eq₀ , refl) | Γ₅' , A' ∷ Ω' , refl , refl , refl | inj₁ (Ξ , Ξ' , refl , refl , refl)
  rewrite ++?-alt-eq₂ (∘fma A') (∘cxt Γ₅') (∘cxt Ω') (∘cxt Ξ) | split-map-eq ∘fma Γ₅' (A' ∷ Ω') | ++?-eq₁ (∘cxt Γ₀) (∘cxt (Ξ ++ Ξ')) (∘cxt Δ) | ++?-eq₁ (∘cxt Ξ) (∘cxt Ξ') (∘cxt Δ) =
    congfocl refl (congfocr refl (cong (unfoc _) (cong (MMF.⊸r⋆⇑ Δ)
      (trans {!only-rf⇑N-lf⇑P!}
             {!!}))))  
only-rf-fN-lf-fP .(Γ₅' ++ A' ∷ Ω') Γ₀ Γ₁ Γ₂ .(A'' ∷ Ξ ++ Ξ') (focl {_} {_} {_} {_} {.(map _ Γ₅')} q lf (focr {_} {_} {_} {_} {_} {.((∘ , A') ∷ map _ Ω' ++ map (λ z → ∘ , z) Γ₁ ++ (∘ , A'') ∷ map (λ z → ∘ , z) Ξ)} {.(map (λ z → ∘ , z) Ξ')} (just (M , m)) rf (unfoc ok f) refl refl ξ₁) refl refl ξ) refl ℓ | inj₁ (.(map (λ z → ∘ , z) Γ₁ ++ (∘ , A'') ∷ map (λ z → ∘ , z) Ξ) , refl , refl) | inj₂ (.(∘ , A') , .(map _ Ω') , eq₀ , refl) | Γ₅' , A' ∷ Ω' , refl , refl , refl | inj₂ (A'' , Ξ , Ξ' , refl , refl , refl)
  rewrite ++?-alt-eq₂ (∘fma A') (∘cxt Γ₅') (∘cxt Ω') (∘cxt (Γ₁ ++ A'' ∷ Ξ)) | split-map-eq ∘fma Γ₅' (A' ∷ Ω') | ++?-eq₁ (∘cxt Γ₀) (∘cxt Γ₁) (∘cxt (A'' ∷ Ξ ++ Ξ')) | ++?-eq₂ (∘fma A'') (∘cxt Γ₁) (∘cxt Ξ) (∘cxt Ξ') | split-map-eq ∘fma Ξ Ξ' =
    congfocl refl (congfocr refl (cong (unfoc _) (cong (MMF.⊸r⋆⇑ (_ ∷ Ξ ++ Ξ'))
      (trans {!!}
             {!!}))))
...
only-rf-fN-lf-fP Γ Γ₀ .(Ξ ++ Ξ' ++ Ξ'') Γ₂ Δ (focl {_} {_} {_} {_} {.(map _ Γ ++ map (λ z → ∘ , z) Ξ)} q lf (focr {Γ₀ = .(map (λ z → ∘ , z) Ξ')} {.(map (λ z → ∘ , z) Ξ'' ++ map (λ z → ∘ , z) Δ)} (just (M , m)) rf (unfoc ok f) refl refl ξ₁) refl refl ξ) refl ℓ | inj₁ (.(map (λ z → ∘ , z) Ξ ++ map (λ z → ∘ , z) Ξ') , refl , refl) | inj₁ (.(map (λ z → ∘ , z) Ξ) , refl , refl) | inj₁ (Ξ , Ξ' , Ξ'' , refl , refl , refl , refl)
  rewrite ++?-alt-eq₁ (∘cxt Γ) (∘cxt Ξ) (∘cxt Ξ') | ++?-eq₁ (∘cxt Γ₀) (∘cxt (Ξ ++ Ξ' ++ Ξ'')) (∘cxt Δ) | ++?-eq₁ (∘cxt (Ξ ++ Ξ')) (∘cxt Ξ'') (∘cxt Δ) =
    congfocl refl (congfocr refl (cong (unfoc _) (cong (MMF.⊸r⋆⇑ Δ)
      (trans {!!}
             {!!}))))
only-rf-fN-lf-fP Γ Γ₀ .(Ξ ++ Ξ') Γ₂ .(A' ∷ Ξ'' ++ Ξ''') (focl {_} {_} {_} {_} {.(map _ Γ ++ map (λ z → ∘ , z) Ξ)} q lf (focr {Γ₀ = .(map (λ z → ∘ , z) Ξ' ++ (∘ , A') ∷ map (λ z → ∘ , z) Ξ'')} {.(map (λ z → ∘ , z) Ξ''')} (just (M , m)) rf (unfoc ok f) refl refl ξ₁) refl refl ξ) refl ℓ | inj₁ (.(map (λ z → ∘ , z) Ξ ++ map (λ z → ∘ , z) Ξ' ++ (∘ , A') ∷ map (λ z → ∘ , z) Ξ'') , refl , refl) | inj₁ (.(map (λ z → ∘ , z) Ξ) , refl , refl) | inj₂ (inj₁ (Ξ , Ξ' , A' , Ξ'' , Ξ''' , refl , refl , refl , refl , refl))
  rewrite ++?-alt-eq₁ (∘cxt Γ) (∘cxt Ξ) (∘cxt (Ξ' ++ A' ∷ Ξ'')) | ++?-eq₁ (∘cxt Γ₀) (∘cxt (Ξ ++ Ξ')) (∘cxt (A' ∷ Ξ'' ++ Ξ''')) | ++?-eq₂ (∘fma A') (∘cxt (Ξ ++ Ξ')) (∘cxt Ξ'') (∘cxt Ξ''') | split-map-eq ∘fma Ξ'' Ξ''' =
    congfocl refl (congfocr refl (cong (unfoc _) (cong (MMF.⊸r⋆⇑ (_ ∷ Ξ'' ++ Ξ'''))
      (trans {!!}
             {!!}))))
only-rf-fN-lf-fP Γ Γ₀ Γ₁ Γ₂ .(A' ∷ Ξ ++ Ξ' ++ Ξ'') (focl {_} {_} {_} {_} {.(map _ Γ ++ map (λ z → ∘ , z) Γ₁ ++ (∘ , A') ∷ map (λ z → ∘ , z) Ξ)} q lf (focr {Γ₀ = .(map (λ z → ∘ , z) Ξ')} {.(map (λ z → ∘ , z) Ξ'')} (just (M , m)) rf (unfoc ok f) refl refl ξ₁) refl refl ξ) refl ℓ | inj₁ (.((map (λ z → ∘ , z) Γ₁ ++ (∘ , A') ∷ map (λ z → ∘ , z) Ξ) ++ map (λ z → ∘ , z) Ξ') , refl , refl) | inj₁ (.(map (λ z → ∘ , z) Γ₁ ++ (∘ , A') ∷ map (λ z → ∘ , z) Ξ) , refl , refl) | inj₂ (inj₂ (A' , Ξ , Ξ' , Ξ'' , refl , refl , refl , refl))
  rewrite ++?-alt-eq₁ (∘cxt Γ) (∘cxt (Γ₁ ++ A' ∷ Ξ)) (∘cxt Ξ') | ++?-eq₁ (∘cxt Γ₀) (∘cxt Γ₁) (∘cxt (A' ∷ Ξ ++ Ξ' ++ Ξ'')) | ++?-eq₂ (∘fma A') (∘cxt Γ₁) (∘cxt (Ξ ++ Ξ')) (∘cxt Ξ'') | split-map-eq ∘fma (Ξ ++ Ξ') Ξ'' =
    congfocl refl (congfocr refl (cong (unfoc _) (cong (MMF.⊸r⋆⇑ (_ ∷ Ξ ++ Ξ' ++ Ξ''))
      (trans {!!}
             {!!}))))
...
... | inj₁ (Λ , refl , eq') with split-map++ Λ Γ₄ Γ₁ Δ (sym eq')
only-rf-fN-lf-fP Γ Γ₀ .(Ω ++ Ω') Γ₂ Δ (focr {_} {_} {_} {_} {_} {.(map _ Γ ++ map (λ z → ∘ , z) Ω)} {.(map (λ z → ∘ , z) Ω' ++ map (λ z → ∘ , z) Δ)} (just (M ⊸ _ , m)) rf (unfoc ok f) refl refl ξ) refl ℓ | inj₁ (.(map (λ z → ∘ , z) Ω) , refl , refl) | inj₁ (Ω , Ω' , refl , refl , refl)
  rewrite ++?-eq₁ (∘cxt Γ₀) (∘cxt (Ω ++ Ω')) (∘cxt Δ) | ++?-eq₁ (∘cxt Ω) (∘cxt Ω') (∘cxt Δ) =
    congfocl refl (congfocr refl (cong (unfoc _) (cong (MMF.⊸r⋆⇑ Δ) {!!})))
only-rf-fN-lf-fP Γ Γ₀ Γ₁ Γ₂ .(A ∷ Ω ++ Ω') (focr {_} {_} {_} {_} {_} {.(map _ Γ ++ map (λ z → ∘ , z) Γ₁ ++ (∘ , A) ∷ map (λ z → ∘ , z) Ω)} {.(map (λ z → ∘ , z) Ω')} (just (M , m)) rf (unfoc ok f) refl refl ξ) refl ℓ | inj₁ (.(map (λ z → ∘ , z) Γ₁ ++ (∘ , A) ∷ map (λ z → ∘ , z) Ω) , refl , refl) | inj₂ (A , Ω , Ω' , refl , refl , refl)
