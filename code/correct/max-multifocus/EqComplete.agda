...
untag-seq≡ (⊸r f) refl = cong ⊸r {!untag-seq≡ f refl!}
untag-seq≡ (Il q f) refl = cong (Il q) (untag-seq≡ f refl)
untag-seq≡ (⊗l q f) refl = cong (⊗l q) (untag-seq≡ f refl)
untag-seq≡ (foc s q f) refl = congfoc {!!}

only-rf⇑N-at≡ : {Δ : TCxt} (Δ₀ : TCxt) {Δ₁ : Cxt} (Γ : Cxt) {B Q X : Fma} {x : isAt X}
  (n : isNeg (Γ ⊸⋆ B)) (q : isPosAt Q)
  (rf : just (Γ ⊸⋆ B , neg→negat n) MMF.⇛rf Δ₁ ； Q) 
  (f : (∘ , just X) MMF.∣ Δ ⇑ (∘ , B))
  (eq : Δ ≡ Δ₀ ++ ∘cxt Γ) → 
 ------------------------------------
  only-rf⇑N Δ₀ Γ n q rf f eq
    ≡ foc (at→negat x) q (focl (at→posat x) blurl (focr {Γ₁ = ∘cxt Δ₁} (just (_ , (neg→negat n))) rf (unfoc (inj₂ n) (MMF.⊸r⋆⇑ Γ (untag-seq {Γ = Δ₀ ++ ∘cxt Γ} (MMF.subst⇑ f eq)))) refl refl tt) refl refl tt)
...
only-rf⇑N-at≡ Δ₀ Γ n q rf (⊸r f) refl =
  trans (only-rf⇑N-at≡ Δ₀ (Γ ∷ʳ _) n q rf f refl)
        (congfoc (congfocl refl (congfocr refl (cong (unfoc (inj₂ n)) {!!}))))
only-rf⇑N-at≡ Δ₀ Γ {X = ` X} n q rf (foc s q₁ f) refl = 
...
only-rf-fN-at≡ ._ Γ n q q' rf (focl {Γ₀ = []} q₁ blurl (focr {Γ₀ = Γ₀} (just (M ⊸ M₁ , m)) rf₁ (unfoc (inj₂ _) f) refl refl ξ₁) refl refl ξ) refl | inj₁ (._ , p , refl) | inj₁ (Ω , refl , refl)
  rewrite only-rf⇑N-at≡ (∘tcxt Γ₀) [] tt q' rf₁ f refl | untag-seq≡ {Γ = Γ₀}{Γ' = ∘tcxt Γ₀} f refl = refl
...
only-rf-fN-at≡ Δ₀ ._ n q q' rf (focl {Γ₀ = []} q₁ blurl (focr (just (M ⊸ M₁ , m)) rf₁ (unfoc (inj₂ _) f) refl refl ξ₁) refl refl ξ) refl | inj₁ (.Δ₀ , p , refl) | inj₂ (._ , ._ , refl , eq') | A ∷ Ω' , Γ₁' , refl , refl , refl =
  congfocl refl (congfocr refl (cong (unfoc (inj₂ tt)) (cong (MMF.⊸r⋆⇑ (_ ∷ Ω' ++ Γ₁'))
    (trans (only-rf⇑N-at≡ (∘tcxt Δ₀ ++ ∘cxt (A ∷ Ω')) [] tt q' rf₁ f refl)
...
only-rf-fN-at≡ Δ₀ Γ n q q' rf (focr (just (M , m)) rf₁ (unfoc ok f) eq₁ eq' ξ) eq = {!f imp!}
...
max≗⇓r₁ (early-lf Δ r) refl = {!!}
max≗⇓r₁ (early-lf-at Δ r) refl = {!!}
max≗⇓r₁ {Γ₀ = Γ₀} {M = M ⊸ M₁} {q = q} (blurl-at {f = f}) {h} refl =
  sym (trans (only-rf⇑N-at≡ (∘cxt Γ₀) [] tt q (max-rf h) (max f) refl)
