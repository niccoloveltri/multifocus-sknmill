...
MF-only-rf-at _ x q rf (foc s q₁ (focl q₂ lf f refl refl ξ)) = {!f!}
MF-only-rf-at Δ₀ x q rf (foc s q₁ (focr s₁ rf₁ f eq eq' ξ)) = {!!}

MF-only-rf : {S : Stp} (Δ₀ : TCxt) {Δ₁ : Cxt} {M Q : Fma}
             (m : isNegAt M) (q : isPosAt Q)
             (rf : just (M , m) MF.⇛rf Δ₁ ； Q) 
             (f : (∘ , S) MMF.∣ Δ₀ ⇑ (∘ , M)) → S MF.∣ untag-cxt Δ₀ ++ Δ₁ ⇑ Q
MF-only-rf Δ₀ {M = ` X} m q rf f = MF-only-rf-at Δ₀ tt q rf f
MF-only-rf Δ₀ {M = A ⊸ B} m q rf f = MF-only-rfN (untag-cxt Δ₀) [] tt q rf (untag⇑ f)
