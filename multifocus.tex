...
Invertible rules and passivation are easily proved to be admissible in $\up$ phase (with a general formula as succedent), and similarly $\IR$ and  $\ax$.
\begin{proposition}
  The following rules are admissible:
  \[
  \small
...
\begin{proposition}\label{prop:lolliL}
  ...
  \end{array}
  \]
  where, in the second rule, $\vec{\Gamma} = \Gamma_1,\dots,\Gamma_n$ is a list of contexts and $\vec{A} = A_1,\dots,A_n$ is a list of formulae, both non-empty.
\end{proposition}
\begin{proof}
  Proving the admissibility of the second rule proceeds by inspecting the polarity of formula $B$ and then by induction on the structure of the derivation $g : B \mid \Delta \up C$. When $B$ is positive, we need to strengthen the statement for the induction to succeed. So we prove the admissibility of the more general rule:
  \[
  \small
  \begin{array}{c}
...
%  Here $\vec{D}$ is a possibly empty list of formulae, and $\mathsf{hd}$ and $\mathsf{tl}$ are the head and tail operations on non-empty lists.
  The additional assumption $\ell : \L(S\mid \Delta_0;B)$ %serve as accumulators for dealing with the case in which $g$ is a left-invertible rule or passivation. These assumptions
  allows to state that the proof of the second premise $g$ is a subderivation of sequent $B \mid \Delta_1 \vdash C$ obtained by repeated applications of left-invertible rules and passivation. More explicitly:
...
%  \def\arraystretch{2.5}
  \begin{array}{c}
    ...
    \\[6pt]
    \infer[\unfoc]{\dbox{$S$}_{\,b} \mid \Gamma \dn \dbox{$A$}_{\,b'}}{
      S^{c(b)} \mid \green{\Gamma} \up A^{c(b')}
      &
      (b = 1 \supset S = P)
      &
      (b' = 1 \supset A = N)
